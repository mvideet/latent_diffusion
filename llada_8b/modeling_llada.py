...
class LLaDABlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        # This attribute is used to optionally store a function for activation checkpointing,
        # which helps save memory during training by recomputing activations in the backward pass.
...
class LLaDABlockGroup(nn.ModuleList):
    """
    LLaDABlockGroup is a container for a group of transformer blocks (LLaDABlock instances)
    that are processed sequentially as a unit within the model. This class enables efficient
    grouping of layers for purposes such as activation checkpointing, memory management, and
    modularity in large transformer architectures.

    Key features:
    - Inherits from nn.ModuleList, so it behaves like a list of modules (blocks), but is also a PyTorch module.
    - Supports activation checkpointing strategies, allowing selective recomputation of activations
      to save memory during training. The strategy can be set to checkpoint every Nth block, or all blocks.
    - Handles passing of attention bias and past key/value states to each block in the group.
    - Returns the output tensor and, if requested, a list of attention key/value caches from each block.

    Args:
        config (ModelConfig): Model configuration.
        layer_offset (int): The starting index of this group within the full set of layers.
        modules (Optional[Iterable[nn.Module]]): The blocks to include in this group.

    Typical usage:
        - The model divides all transformer blocks into groups (e.g., for block_group_size=4, each group has 4 blocks).
        - Each group is an instance of LLaDABlockGroup, and the model iterates through groups in forward().
        - This enables efficient memory and compute management for large models.
    """
